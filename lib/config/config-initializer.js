...
function processAnswers(answers) {
    var config = {parserOptions: {}, rules: {}, env: {}};
    if (answers.es6) {
        config.env.es6 = true;
    }
    if (answers.commonjs) {
    ...
    if (answers.source === "auto") {
        ...
        bar.tick(2); // Getting source code can take a long time, so tick ahead 2

        var registry = autoconfig.lintWithConfigs(sourceCodes, config, function(total) {
            bar.tick((BAR_TOTAL - 2) / total); // Subtract out ticks used at beginning
        });
        ...
        console.log("\n"); // Make sure there's at least one newline after progress bar
    }
    return config;
}
